...
describe('Download Transfer coverage gaps', () => {
    let mockRequest: any;
    ...
    it('should handle end callback and prevent double completion', () => {
        ...
        // We need to trigger the end handler and call the callback passed to stream.end
        const endHandler = mockResponse.on.mock.calls.find((c: any) => c[0] === 'end')[1];
        endHandler();
        ...
        // Calling it again should be ignored due to `settled` flag
        streamEndCb();
