...
describe('ModManager.verifyDeployment', () => {
    ...
    it('re-deploys an enabled mod whose files a game update removed', async () => {
        ...
        expect(fs.link).toHaveBeenCalledWith(`${MOD_FOLDER}/goku.pak`, expect.stringContaining(`${DEPLOYED_PAK}.kamisama-`));
        expect(fs.rename).toHaveBeenCalledWith(expect.stringContaining(`${DEPLOYED_PAK}.kamisama-`), DEPLOYED_PAK);
    ...
    it('preserves all files without catalog ownership, including numbered manual paks', async () => {
        ...
        (fs.access as any).mockResolvedValue(undefined);
        (fs.readdir as any).mockImplementation(async (dir: string) => {
            if (String(dir) === PAKS_DIR)
                return ['001_goku.pak', '004_stale.pak', 'manual.pak'];
            return [];
        });
        const result = await modManager.verifyDeployment();
        expect(result.removedOrphans).toBe(0);
        expect(fs.unlink).not.toHaveBeenCalledWith(path.join(PAKS_DIR, '004_stale.pak'));
