...
export class DownloadManager extends EventEmitter {
  ...
  private processDownload(id: string) {
    ...
    startDownloadTransfer(item, {
      ...
      onFailure: fail,
      onComplete: () => {
        item.state = ['install', 'update'].includes(String(item.context?.type)) ? 'installing' : 'completed';
        item.progress = 100;
        item.speed = 0;
        cleanup();
        this.emitUpdate();
  ...
  cancelDownload(id: string) {
    const item = this.downloads.get(id);
    if (!item || !['queued', 'progressing', 'paused'].includes(item.state)) return;
  ...
  completeInstallation(id: string) {
    const item = this.downloads.get(id);
    if (!item || item.state !== 'installing') return;
    item.state = 'completed';
    this.emitUpdate();
    this.mainWindow?.webContents.send('download-scan-finished');
  }
  clearCompleted() {
